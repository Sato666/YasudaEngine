...
D3D11_TEXTURE2D_DESC Texture::MakeTexDesc(DXGI_FORMAT format, UINT width, UINT height)
{
	...
	desc.Usage = D3D11_USAGE_DEFAULT;				// GPU による読み取りと書き込みのアクセスを必要とする
	desc.Format = format;							// テクスチャの形式
	desc.BindFlags = D3D11_BIND_SHADER_RESOURCE;	// テクスチャをシェーダーステージにバインド
	desc.Width = width;								// 解像度の幅
	desc.Height = height;							// 解像度の高さ
	desc.MipLevels = 1;								// ミニマップレベル最大数
	desc.ArraySize = 1;								// 配列内のテクスチャの数
	desc.SampleDesc.Count = 1;						// マルチサンプリングのサンプル数(未使用は1)
	return desc;
}
...
HRESULT Texture::CreateResource(D3D11_TEXTURE2D_DESC& desc, const void* pData)
{
	...
	// テクスチャ作成
	D3D11_SUBRESOURCE_DATA data = {};	// サブリソースの初期化
	data.pSysMem = pData;				// 初期化データへのポインター
	data.SysMemPitch = desc.Width * 4;	// テクスチャの1行目の先頭から次の行までのバイト単位の距離
	// 生成(サブリソースがが存在しない場合空のデータを渡す)
...
HRESULT RenderTarget::CreateFromScreen()
{
	...
	rtvDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;		// ImGuiが暗めの描画なのでそれに合わせる　DXGI_FORMAT_R8G8B8A8_UNORM_SRGBはγ値の情報があり、-1/2乗したら変換できる
