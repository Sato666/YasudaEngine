...
void AnimationModel::Load(const char* FileName)
{
	...
	assert(m_AiScene);	// エラー処理

	m_VertexBuffer = new ID3D11Buffer * [m_AiScene->mNumMeshes];	// 頂点バッファ生成
	m_IndexBuffer = new ID3D11Buffer * [m_AiScene->mNumMeshes];		// インデックスバッファ生成
	...
	for (unsigned int m = 0; m < m_AiScene->mNumMeshes; m++)
	{
		...
		// 頂点バッファ生成
		{
			...
			{
				// 値の吸出し
				aiVector3D pos = mesh->mVertices[v];			// 頂点座標
				aiVector3D uv = mesh->HasTextureCoords(0) ?
					mesh->mTextureCoords[0][v] : zero;			// UV座標
				aiVector3D normal = mesh->HasNormals() ?
					mesh->mNormals[v] : zero;					// 法線

				// 吸いだした値をセット
				vertex[v].Position = Vector3(pos.x, pos.y, pos.z);
				vertex[v].Normal = Vector3(normal.x, normal.y, normal.z);
				vertex[v].TexCoord = Vector2(uv.x, uv.y);
